...
let trinkets = await readFile("trinkets.json", "utf-8");
trinkets = JSON.parse(trinkets);
let achievements = [];
tables.forEach((table) => {
  ...
  rows.forEach((row) => {
    ...
    achievements.push({
      ...
      link: link
        ? "https://bindingofisaacrebirth.fandom.com" + link.getAttribute("href")
        : null,
      ...
      id: parseInt(
        columns[4].innerText.replaceAll("&#32;", "").replaceAll("\n", "")
      ),
    });
  });
});
achievements = achievements.map((a) => {
  ...
  if (a.link) {
    ...
    let trinket = trinkets.find((i) => i.link == a.link);
    if (item) a.item = item;
    if (trinket) a.trinket = trinket;
  } else delete a.link;
  return a;
});
